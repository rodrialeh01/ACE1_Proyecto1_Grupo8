...
app.post('/alarmaAntirrobos',(req,res) => {
    ...
    if(estado == 0){
        ...
        response = {
            "Mensaje": "Se activó la alarma del espacio "+str(data["posicion"])+" del nivel "+str(data["nivel"]),
            "Estado": 1
        }
    }else if(estado == 1){
        alarmas[data["nivel"]-1][data["posicion"]-1] = 0;
        response = {
            "Mensaje": "Se desactivó la alarma del espacio "+str(data["posicion"])+" del nivel "+str(data["nivel"]),
            "Estado": 0
        }
    }
    res.json(response);
